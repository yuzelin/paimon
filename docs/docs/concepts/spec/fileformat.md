...
It uses the envelope encryption practice, where file parts are encrypted with “data encryption keys” (DEKs), and the DEKs are encrypted with “master encryption keys” (MEKs). 
...
Here is an example of how to use Parquet encryption:

First, you need to configure the global settings in your environment, for example in your `core-site.xml`.
The [InMemoryKMS](https://github.com/apache/paimon/blob/master/paimon-format/src/main/java/org/apache/paimon/format/parquet/crypto/InMemoryKMS.java#L35) class provided the in-memory Parquet encryption.

```xml
<property>
    <name>parquet.encryption.kms.client.class</name>
    <value>org.apache.paimon.format.parquet.crypto.InMemoryKMS</value>
</property>
<property>
    <name>parquet.encryption.key.list</name>
    <value>footerKey:AAECAwQFBgcICQoLDA0ODw==, columnKey:AAECAwQFBgcICQoLDA0ODw==</value>
</property>
<property>
...
Then, create table with encryption properties, and when you write to or read Parquet, encryption and decryption are performed automatically
on parquet footer and col a, col b.

```sql
-- encrypt footer by footerKey, encrypt col a and col b by columnKey
