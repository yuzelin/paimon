  ...
  override def pullUpAggFilter(plan: LogicalPlan): LogicalPlan = {
    ...
    plan match {
      case agg @ Aggregate(
            ...
            hint) if groupingExpressions.isEmpty && agg.containsPattern(AGGREGATE_EXPRESSION) =>
      ...
      case agg @ Aggregate(
        ...
        Aggregate(groupingExpressions, newAggExprs, removeFilterInV2ScanRelation(child), hint)
