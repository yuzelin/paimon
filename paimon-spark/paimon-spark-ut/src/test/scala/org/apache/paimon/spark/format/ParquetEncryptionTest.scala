...
class ParquetEncryptionTest extends PaimonSparkTestBase {
  ...
  val encryptionConf: Map[String, String] = Map(
  ...
  test("Paimon encryption: read and write with encryption") {
    withEncryptionConf {
      withTable("t") {
        sql("""
              |CREATE TABLE t (a INT, b STRING, pt STRING) USING paimon TBLPROPERTIES (
              |'file.format' = 'parquet',
        ...
        sql("INSERT INTO t VALUES (1, 'a', 'p1')")
        checkAnswer(sql("SELECT * FROM t"), Seq(Row(1, "a", "p1")))
        ...
        Seq((2, "b", "p2"))
          ...
          .mode("append")
          .insertInto("t")
        checkAnswer(sql("SELECT * FROM t"), Seq(Row(1, "a", "p1"), Row(2, "b", "p2")))
      }
    }
  }
  ...
  test("Paimon encryption: read and write dataframe with encryption") {
    withEncryptionConf {
      withTable("t") {
        val data = Seq((1, "a", "p")).toDF("a", "b", "pt")
  ...
  private def assertFooterEncryptionType(filePath: Path, expert: String): Unit = {
    val reader =
      ParquetUtil.getParquetReader(
        ...
        new Options(encryptionConf.asJava))
